...
For stricter semantic-token mapping, use the packaged [`semantic-token-settings.json`]({{{ repository }}}/blob/main/packages/zed/semantic-token-settings.json) file. It is a machine-readable settings fragment intended for dotfile tools such as chezmoi. The mapping is documented in [`docs/ZED_SEMANTIC_TOKENS.md`]({{{ repository }}}/blob/main/docs/ZED_SEMANTIC_TOKENS.md).
