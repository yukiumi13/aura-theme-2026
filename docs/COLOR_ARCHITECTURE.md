...
`createAuraPalette(family, baseOverride?, appearance = 'dark', semanticOverride?)` selects the neutral and semantic palette for an appearance. Existing callers continue to use the dark palette. `schemes/light.ts` explicitly selects `auraLightFamily` and the light appearance, then uses the same role resolver and template-variable adapter as the dark variants.
...
The shared syntax role resolver is `roles/create-aura-syntax.ts`; `semantic.interaction` does not enter this mapping. This is a function within the existing role layer, not an additional algorithm or palette layer. Aqua supplies its own type/control accents and appearance-specific semantic colors independently of its UI family. Each variant is judged on its own visual quality; sharing role mappings does not require matching colors with the default theme. Older accent variants retain their existing syntax families. The shared TextMate map assigns `constant.numeric` to `syntaxNumber`, matching semantic highlighting when no language server supplies number tokens. Pre-existing TextMate and semantic syntax mappings remain unchanged.

### Brand mint versus text ink

`auraMint` in `source/aura.ts` owns the original `#61FFCA` anchor shared by both appearances. The semantic brand palette exposes four distinct purposes: `mint` (identity), `mintText` (readable ink), `mintHover` (legacy fallback for interaction feedback), and `onMint` (contrast foreground). Default light actions and badges use the original color, while syntax strings/tags, success diagnostics, and terminal green use the ink. Adjusting one must not silently change the others.

Actions and badges have explicit foreground/background pairs. Primary, secondary, and extension-marketplace buttons use transparent decorative borders in the light appearance; the purple keyboard focus indicator remains. A bright fill uses dark ink for both normal and hover states because VS Code does not provide a separate button-hover foreground. Remote/SSH status items follow the same mint-fill, dark-ink pairing. Ordinary compact status items retain a neutral hover surface. `semantic.interaction.accent` supplies aqua action/Remote hover independently of the normal mint fill. Dark UI interaction emphasis uses the same aqua accent; dark normal action fills use the family accent for readable dark labels.

In the default light theme and both Aqua variants, `ui.interactionForeground` keeps ordinary hovered/focused list labels, menu items, tab titles, and breadcrumbs neutral. Other dark themes use aqua interaction text. `ui.highlight.foreground` is reserved for search-match and other semantic emphasis and uses purple in the light appearance. Light neutral hover fills are opaque so they do not pick up a tint from the underlying surface; links remain purple on hover. Status decorations such as Git additions retain their own semantic colors during ordinary hover states.
